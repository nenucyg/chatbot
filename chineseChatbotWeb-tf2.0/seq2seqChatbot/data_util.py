...
if not os.path.exists(conv_path):
	raise FileNotFoundError(f"未找到训练语料文件: {conv_path}。请先运行 python3 download_corpus.py 下载语料。")
if os.path.getsize(conv_path) == 0:
	raise ValueError(f"训练语料文件为空: {conv_path}。请先运行 python3 download_corpus.py 重新下载语料。")
if os.path.getsize(conv_path) == 0:
